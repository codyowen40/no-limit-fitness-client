...
const backupPath = path.join(
  __dirname,
  "src",
  `App-before-stable-messages-nav-final-${Date.now()}.jsx`
);

fs.copyFileSync(appPath, backupPath);

// Remove old runtime Messages nav effect if it exists.
source = source.replace(
  /\n\s*useEffect\(\(\)\s*=>\s*\{\s*\n\s*function getCleanNavLabel\(button\)[\s\S]*?ensureRuntimeMessagesNavButton[\s\S]*?\n\s*\},\s*\[activeTab,\s*portalMode\]\);\s*/g,
  "\n"
);

// Remove old injected Messages buttons.
source = source.replace(
...
// Add Messages to visible tab arrays when Tracker jumps straight to Exercises.
source = source.replace(
  /(["']Tracker["']\s*,\s*)(?!["']Messages["'])(["']Exercises["'])/g,
  `$1"Messages",\n        $2`
);

// Find the app nav block.
...
const navAlreadyHasMessages =
  />\s*Messages\s*</.test(navBlock) ||
  /Messages\s*\{/.test(navBlock) ||
  /name:\s*["']Messages["']/.test(navBlock) ||
  /label:\s*["']Messages["']/.test(navBlock);

if (!navAlreadyHasMessages) {
  const messagesButton = `
          <button
            type="button"
            data-nlf-stable-messages-nav="true"
...
console.log("Stable Messages nav fix installed.");
console.log("There should now be exactly one visible Messages nav button.");
