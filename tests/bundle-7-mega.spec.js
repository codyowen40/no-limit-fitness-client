...
function buildSeedState() {
  ...
  return {
    ...
    backendSettings: {
      ...
      notes: "Bundle 7 confirms backend-ready settings stay local until Supabase import is intentional.",
    },
  };
}
...
test.describe("No Limit Fitness Bundle 7 mega regression", () => {
  test("loads a full seeded local app state and keeps the full storage shape", async ({ page }) => {
    ...
    expect(stored.backendSettings.emailProvider).toBe("Supabase + Resend");
  });

  test("shows seeded plans, clients, messages, logs, and backend settings across demo tabs", async ({ page }) => {
    await openSeededApp(page);
    ...
    await expectMainText(page, "Supabase Data Bridge Preview");
