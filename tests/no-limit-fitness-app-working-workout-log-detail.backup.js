...
test.describe("No Limit Fitness full app test", () => {
  test("checks full frontend flow including Messages, localStorage refresh, and Activity Center", async ({
    page,
  }) => {
    ...
    await test.step("localStorage keeps clients, plans, logs, messages, and Activity Center read status after refresh", async () => {
      await page.reload();
