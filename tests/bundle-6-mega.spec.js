...
test.describe("No Limit Fitness Bundle 6 mega regression", () => {
  ...
  test("keeps notification preferences and backend-ready settings after refresh", async ({ page }) => {
    await expect(
    ...
    await page.getByLabel("Future Email Provider").selectOption("Backend undecided");
    await page.getByLabel("Backend Status").fill("Bundle 6 settings persisted after refresh.");
    ...
    await expect(page.getByLabel("Future Email Provider")).toHaveValue("Backend undecided");
    await expect(page.getByLabel("Backend Status")).toHaveValue(
