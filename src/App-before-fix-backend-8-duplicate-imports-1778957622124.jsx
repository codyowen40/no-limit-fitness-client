...
import { getCurrentSession,
  ...
  signOutUser } from "./lib/noLimitSupabaseApi";
import { fetchBackendClients,
  fetchBackendPlans,
...
function mapBackendPlanExerciseForApp(exercise) {
  return {
    ...
    repsOrTime: String(
      getBackendValue(exercise, ["repsOrTime", "reps_or_time", "reps", "time", "assignedRepsOrTime", "assigned_reps_or_time"], "")
    ),
    weightGuidance: String(
      getBackendValue(exercise, ["weightGuidance", "weight_guidance", "assignedWeightGuidance", "assigned_weight_guidance"], "")
    ),
    rest: String(getBackendValue(exercise, ["rest", "restPeriod", "rest_period", "assignedRest", "assigned_rest"], "")),
...
function mapBackendPlanDayForApp(day, index) {
  const exercises = Array.isArray(day?.exercises)
    ? day.exercises
    : Array.isArray(day?.planExercises)
      ? day.planExercises
      : [];
...
function mapBackendWorkoutLogForApp(log, clients, plans) {
  ...
  const entries = Array.isArray(log?.entries)
    ? log.entries
    : Array.isArray(log?.workoutEntries)
      ? log.workoutEntries
      : [];
