...
function loadInitialState() {
  ...
  try {
    ...
    const savedPlans = Array.isArray(parsed.savedPlans) ? parsed.savedPlans : [];
...
export default function App() {
  ...
  const [localSaveNotice, setLocalSaveNotice] = useState(
    "Local saving is active. Clients, plans, logs, and messages will stay after refresh."
  );
  ...
  }, [clients, savedPlans, workoutLogs, conversations]);
  ...
  const coachNotifications = useMemo(() => {
    ...
    const messageActivity = conversations.flatMap((conversation) =>
      conversation.messages
        .filter((message) => message.timestamp > 10)
        .map((message) => ({
          title:
            message.sender === "Client" ? "Client sent message" : "Coach sent message",
  ...
  }, [savedPlans, workoutLogs, conversations]);
        ...
        <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
          <div>
            ...
            <p className="mt-5 max-w-3xl text-base leading-7 text-white/70">
              ...
              local messages, and keep data saved after refresh using localStorage.
            </p>
...
function CoachScreen({ notifications, savedPlans, workoutLogs, unreadCoachCount }) {
  return (
    <div>
      <SectionHeader
        eyebrow="Coach Dashboard"
        title="Command Center"
        description="Coach-facing activity updates when a client completes workouts, skips workouts, changes values, substitutes exercises, leaves notes, or sends messages."
      />
  ...
  return (
    <div>
      <SectionHeader
        ...
        description="Send local in-app messages between coach and clients. This is frontend-only for now, with unread indicators saved in React state and localStorage."
      />
              ...
              return (
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {coachUnread > 0 && (
                        <UnreadPill label={`Coach ${coachUnread}`} />
                      )}
                      {clientUnread > 0 && (
